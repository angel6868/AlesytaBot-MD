...
global.conn = makeWASocket(connectionOptions);
    ...
    if (pairingCode && !conn.authState.creds.registered) {
        ...
        if (!!phoneNumber) {
            ...
            numeroTelefono = await question(chalk.bgBlack(chalk.greenBright(`Por favor, escriba su número de WhatsApp 😍\nPor ejemplo: +59176181985 : `)))
            numeroTelefono = numeroTelefono.replace(/[^0-9]/g, '')
            if (!Object.keys(PHONENUMBER_MCC).some(v => numeroTelefono.startsWith(v))) {
                console.log(chalk.bgBlack(chalk.redBright("Comience con el código de país de su número de WhatsApp, Ejemplo: +59176181985")))
...
setInterval(async () => {
  ...
  const bio = `🤖 ᴛɪᴇᴍᴘᴏ ᴀᴄᴛɪᴠᴏ: ${uptime} ┃ 👑  ʙʏ ᴋᴀʀɪᴍ ┃
